// ==================================================================
// St Page Flip (paquete npm "page-flip", build UMD para navegador)
// ------------------------------------------------------------------
// Este archivo se genero con "npm pack page-flip@2.0.7" y se incluyo
// tal cual dentro de esta carpeta para que el libro funcione sin
// conexion a internet (no depende de un CDN externo).
//
// El archivo original viene MINIFICADO (todo en una sola linea, con
// nombres de variables y clases reducidos a una letra). Lo que ves
// aca es el mismo codigo reformateado a multilinea con js-beautify
// y con comentarios agregados para poder ubicarse; el comportamiento
// es identico al original, no se modifico ninguna linea de logica.
//
// Estructura del archivo (de arriba hacia abajo):
//   1) Envoltorio UMD: expone la libreria como "window.St.PageFlip"
//      cuando se carga con <script src="page-flip.browser.js">.
//   2) Clases internas (Page, PageCollection, Helper, FlipCalculation,
//      FlipController, Render, UI, Settings) con sus variantes para
//      modo IMAGENES (las que usamos) y modo HTML.
//   3) Clase publica "PageFlip", que es la que se instancia en
//      nuestro index.html como "new St.PageFlip(elemento, opciones)".
//
// Licencia: MIT (paquete "page-flip" de Nodlik). Repositorio:
...
! function(t, e) {
  ...
  /**
   * ============================================================
   * CLASE BASE: Page  (alias interno minificado: "e")
   * ------------------------------------------------------------
   * Representa el estado de UNA pagina del libro durante el
   * efecto de flip: angulo de curvatura, posicion, area visible
   * y densidad ("soft" mientras se dobla, "hard" para tapas o la
   * ultima pagina de un grupo, que no se curvan).
   * No dibuja nada por si sola: las subclases ImagePage (modo
   * imagenes) y HTMLPage (modo HTML) heredan de aca y agregan el
   * dibujado concreto.
   * ============================================================
   */
  class e {
  ...
  /**
   * ------------------------------------------------------------
   * ImagePage (alias interno: "i") extends Page
   * ------------------------------------------------------------
   * Pagina en modo IMAGENES (el modo que usa este proyecto via
   * loadFromImages/PAGE_IMAGES). Carga una <img> y la dibuja
   * sobre el <canvas> respetando el angulo/recorte calculado
   * por la clase Page. Mientras la imagen no termino de cargar,
   * dibuja un loader circular (drawLoader).
   * ------------------------------------------------------------
   */
  class i extends e {
    constructor(t, e, i) {
  ...
  /**
   * ============================================================
   * CLASE BASE: PageCollection (alias interno: "s")
   * ------------------------------------------------------------
   * Mantiene el arreglo completo de paginas del libro y arma los
   * "spreads": los pares de paginas que se muestran juntas en
   * modo paisaje (libro abierto de a dos hojas) versus una sola
   * pagina por vez en modo retrato. showCover=true hace que la
   * primera pagina quede sola, como tapa.
   * ============================================================
   */
  class s {
    constructor(t, e) {
  ...
  /**
   * ------------------------------------------------------------
   * ImagePageCollection (alias interno: "n") extends PageCollection
   * ------------------------------------------------------------
   * Recibe el arreglo de URLs de imagenes (PAGE_IMAGES en nuestro
   * index.html) y crea una ImagePage por cada una.
   * ------------------------------------------------------------
   */
  class n extends s {
    constructor(t, e, i) {
  ...
  /**
   * ============================================================
   * Helper (alias interno: "h")
   * ------------------------------------------------------------
   * Funciones matematicas estaticas de apoyo: distancia entre dos
   * puntos, largo de un segmento, angulo entre dos lineas, si un
   * punto cae dentro de un rectangulo, rotar un punto, etc. Las
   * usa el motor de calculo del flip (clase FlipCalculation) para
   * saber donde "dobla" la pagina segun la posicion del mouse.
   * ============================================================
   */
  class h {
  ...
  /**
   * ------------------------------------------------------------
   * HTMLPage (alias interno: "r") extends Page
   * ------------------------------------------------------------
   * Variante de pagina para el modo HTML (loadFromHTML): en vez
   * de dibujar una imagen en un canvas, mueve/recorta un elemento
   * DOM real con CSS (clip-path, transform). No la usamos en este
   * proyecto porque trabajamos en modo imagenes, pero queda en el
   * archivo porque es la misma libreria para ambos modos.
   * ------------------------------------------------------------
   */
  class r extends e {
    constructor(t, e, i) {
  ...
  /**
   * ------------------------------------------------------------
   * HTMLPageCollection (alias interno: "o") extends PageCollection
   * ------------------------------------------------------------
   * Version de PageCollection para el modo HTML: arma una
   * HTMLPage por cada elemento del DOM que se le pasa.
   * ------------------------------------------------------------
   */
  class o extends s {
    constructor(t, e, i, s) {
  ...
  /**
   * ============================================================
   * FlipCalculation (alias interno: "a")
   * ------------------------------------------------------------
   * El motor geometrico del efecto: dado el punto donde el
   * usuario esta arrastrando la esquina de la pagina, calcula el
   * angulo de curvatura y los puntos de interseccion que definen
   * el area recortada (la "sombra" de pagina doblada). Es la
   * clase mas matematica de todo el archivo.
   * ============================================================
   */
  class a {
    constructor(t, e, i, s) {
  ...
  /**
   * ============================================================
   * FlipController (alias interno: "g")
   * ------------------------------------------------------------
   * Orquesta el gesto completo de pasar de hoja: arranca el
   * "fold" cuando el usuario toca la esquina, anima el flip
   * automatico (flipNext/flipPrev/flip programatico) apoyandose
   * en FlipCalculation para la geometria, y avisa a Render cuando
   * hay que redibujar cada cuadro de la animacion.
   * ============================================================
   */
  class g {
    constructor(t, e) {
  ...
  /**
   * ============================================================
   * CLASE BASE: Render (alias interno: "l")
   * ------------------------------------------------------------
   * Motor de dibujado. Corre el loop de animacion (requestAnimationFrame),
   * sabe si el libro esta en orientacion "landscape" (doble pagina)
   * o "portrait" (una pagina), y calcula las sombras que dan
   * sensacion de volumen a la hoja doblada. Las subclases
   * CanvasRender (dibuja en <canvas>) y HTMLRender (mueve <div>s)
   * heredan de aca.
   * ============================================================
   */
  class l {
  ...
  /**
   * ------------------------------------------------------------
   * CanvasRender (alias interno: "d") extends Render
   * ------------------------------------------------------------
   * Renderer en modo IMAGENES: es el que efectivamente usamos.
   * Dibuja cada ImagePage sobre un <canvas> unico en cada cuadro
   * de la animacion.
   * ------------------------------------------------------------
   */
  class d extends l {
  ...
  /**
   * ============================================================
   * CLASE BASE: UI (alias interno: "p")
   * ------------------------------------------------------------
   * Conecta los eventos del navegador (mouse, touch, resize) con
   * el FlipController: cuando el usuario hace mousedown/touchstart
   * cerca de una esquina, avisa que arranco un "toque de usuario";
   * al soltar, dispara el flip o lo cancela. Las subclases
   * CanvasUI y HTMLUI arman el contenedor visual concreto.
   * ============================================================
   */
  class p {
    constructor(t, e, i) {
  ...
  /**
   * ------------------------------------------------------------
   * HTMLUI (alias interno: "c") extends UI
   * ------------------------------------------------------------
   * Contenedor de UI para el modo HTML (loadFromHTML): arma un
   * <div class="stf__block"> y le cuelga los elementos reales
   * del documento.
   * ------------------------------------------------------------
   */
  class c extends p {
  ...
  /**
   * ------------------------------------------------------------
   * CanvasUI (alias interno: "u") extends UI
   * ------------------------------------------------------------
   * Contenedor de UI para el modo IMAGENES: es el que usamos.
   * Crea el <canvas class="stf__canvas"> donde CanvasRender dibuja
   * las paginas, y lo re-mide cada vez que cambia el tamano de
   * ventana.
   * ------------------------------------------------------------
   */
  class u extends p {
  ...
  /**
   * ------------------------------------------------------------
   * HTMLRender (alias interno: "w") extends Render
   * ------------------------------------------------------------
   * Renderer en modo HTML: en vez de canvas, mueve <div> reales
   * con CSS y agrega manualmente las capas de sombra
   * (.stf__outerShadow, .stf__innerShadow, etc.) para simular el
   * mismo efecto de volumen que el modo imagenes logra con canvas.
   * ------------------------------------------------------------
   */
  class w extends l {
    constructor(t, e, i) {
  ...
  /**
   * ============================================================
   * Settings (alias interno: "x")
   * ------------------------------------------------------------
   * Define los valores por defecto de TODAS las opciones que se
   * le pueden pasar a "new St.PageFlip(elemento, opciones)" -por
   * ejemplo width, height, size, showCover, usePortrait,
   * maxShadowOpacity, useMouseEvents, etc.- y los combina con las
   * opciones que efectivamente pasamos en nuestro index.html.
   * ============================================================
   */
  class x {
    constructor() {
  ...
  /**
   * ------------------------------------------------------------
   * Auto-inyeccion de CSS (funcion anonima, sin nombre asignable)
   * ------------------------------------------------------------
   * Crea un <style> en el <head> del documento con las clases
   * .stf__parent / .stf__block / .stf__item / sombras, etc. Por
   * eso no hace falta un <link> ni un .css aparte: apenas se
   * carga este .js, ya deja listos los estilos base del libro.
   * ------------------------------------------------------------
   */
  ! function(t, e) {
    void 0 === e && (e = {});
  ...
  /**
   * ============================================================
   * EventObject (clase anonima, base de PageFlip)
   * ------------------------------------------------------------
   * Mini sistema de eventos tipo "on/off/trigger" (parecido a
   * Node's EventEmitter). Es lo que permite hacer, en nuestro
   * index.html:
   *     pageFlip.on('flip', (e) => { ... })
   *     pageFlip.on('init', () => { ... })
   * ============================================================
